...
<code>example.org https://dns.adguard.com/dns-query 94.140.14.14</code>",
                ...
                'data' => array_merge(
                    ...
                    [
                        ...
                        [
                            [
                                'text' => $this->i18n('chat'),
                                'url'  => base64_decode('aHR0cHM6Ly90Lm1lLytSeGloZ3hfaUc0dzNNall5'),
                            ],
                            [
                                'text' => $this->i18n('Creator'),
                                'web_app' => [
                                    'url'  => "https://spbadmin.ru",
                                ]
                            ],
                        ],
                    ],
    ...
    public function cleanDocker()
    {
        ...
        foreach ($r as $v) {
            if (!empty($v['RepoTags'])) {
                foreach ($v['RepoTags'] as $j) {
                    if (preg_match('~^remzoospb/vpnbot~', $j)) {
                        $i[] = $v['Id'];
                        break;
                    }
                }
        ...
        foreach ($r as $v) {
            if (preg_match('~^remzoospb/vpnbot~', $v['Image'])) {
                $c[] = $v['ImageID'];
            }
        }
